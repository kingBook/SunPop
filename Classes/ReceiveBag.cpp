...
bool ReceiveBag::init(){
	...
	std::string strTitle=LocalUtil::getInstance()->getKeyString("lqdlb",true);
	...
	CCMenuItemImage* closeItem = CCMenuItemImage::create("close.png","close.png",this,menu_selector(ReceiveBag::closeCallback));
	closeItem->setPosition(ccp(-170,201));

	CCMenuItemImage* rect_aItem = CCMenuItemImage::create("rect_a.png","rect_a.png",this,menu_selector(ReceiveBag::okCallback));
	rect_aItem->setPosition(ccp(120,120));
	rect_aItem->setOpacity(0);
	CCMenuItemImage* rect_bItem = CCMenuItemImage::create("rect_b.png","rect_b.png",this,menu_selector(ReceiveBag::okCallback));
...
void ReceiveBag::closeCallback(CCObject* pSender){
	...
	if(_isContinueGame) MyGame::getInstance()->continueGame();
	removeAllChildrenWithCleanup(true);
	removeFromParentAndCleanup(true);
	CCLOG("b %s",_isContinueGame?"isContinueGame":"no ContinueGame");
	//
	if(MyGame::getInstance()->getPause()){
...
#endif
	//MyGame::getInstance()->gamePay(12,"bigBag","",this,pay_cb(ReceiveBag::payCallback),220);
}
